...
class EndoSLAMDataset(Dataset):
    def __init__(self, root_dir, data_transform=None,
        ...
        self.file_tree = os.listdir(os.path.join(root_dir))     
        # sort color image file list
        self.file_tree.sort(key=lambda x: int(x.split('_')[1].split('.')[0]))
        self.file_tree  = self.file_tree[:1548]
...
########################################################################################################################
    ...
    def _get_depth_file(self, session,image_file):
        ...
        depth_name = '0000'+ image_file.split('_')[1].split('.')[0]+'_depth.npy'
    ...
    def _get_pose_file(self, session, image_file):
        ...
        pose_name = 'extrinsic_0000' + image_file.split('_')[1].split('.')[0] + '.txt'
    ...
    def __getitem__(self, idx):
        ...
        intr_path = os.path.join('/home/ilsr/WPeng/endoslam-test/param/intrinsic.txt') 
