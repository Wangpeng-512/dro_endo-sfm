...
def read_files(directory, ext=('.png', '.txt'), skip_empty=True):
    files = defaultdict(list)
    for entry in os.scandir(directory):
        relpath = os.path.relpath(entry.path, directory)
        if entry.is_dir():
            image_path = os.path.join(entry.path, 'images')
            d_files = read_files(image_path, ext=ext, skip_empty=skip_empty)
            if skip_empty and not len(d_files):
                continue
            files[relpath + '/color'] = d_files[image_path]
        elif entry.is_file():
...
def read_npy_depth(file, depth_type):
    """Reads a .npz depth map given a certain depth_type."""
...
class BlenderScannetDataset(Dataset):
    def __init__(self, root_dir, split, data_transform=None,
        ...
        if source == 'Folder':
            ...
            with open(os.path.join('./', self.split), "r") as f:
...
########################################################################################################################
    ...
    def _read_depth(self, depth_file):
        ...
        if self.depth_type in ['velodyne']:
            return read_npy_depth(depth_file, self.depth_type)
    ...
    def _get_depth_file(self, image_file):
        ...
        depth_file = image_file.replace('images','depth')
        return depth_file
    ...
    def __getitem__(self, idx):
        ...
        if self.with_depth:
            filename = '0000' + filename.split('.')[0] + '_depth.npy'
