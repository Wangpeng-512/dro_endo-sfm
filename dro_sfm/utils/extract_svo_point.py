...
class PixelSelector:
    def __init__(self):
        ...
        self.so_path = 'dro_sfm/utils/libtest2.so'
        self.lib = ctypes.cdll.LoadLibrary(self.so_path)
        self.c_float_p = ctypes.POINTER(ctypes.c_float)
        self.c_func = self.lib.main
        self.c_func.argtypes = [ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_float), ctypes.c_int, ctypes.c_int, ctypes.POINTER(ctypes.c_float)]

    def extract_points(self, image):
        image = cv2.cvtColor(image, cv2.COLOR_RGB2GRAY)
        image = cv2.resize(np.uint8(image * 255), (960, 320))
        data_p = image.astype(np.float32).ctypes.data_as(self.c_float_p)

        result = np.zeros((6500, 2)).astype(np.float32)
        result_p = result.ctypes.data_as(self.c_float_p)

        point_num = self.c_func(960, 320, data_p, 6000, 6500, result_p)
