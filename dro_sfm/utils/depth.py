...
from dro_sfm.utils.image import load_image, gradient_x, gradient_y, flip_lr, interpolate_image
from dro_sfm.utils.types import is_seq, is_tensor
...
def compute_depth_metrics(config, gt, pred, use_gt_scale=True):
    ...
    return torch.tensor([metric / batch_size for metric in
        [abs_rel, sq_rel, rmse, rmse_log, a1, a2, a3, SILog, iabs_diff]]).type_as(gt)
...
def compute_depth_metrics_demon(config, gt, gt_pose, pred, use_gt_scale=True):
    ...
    for pred_i, gt_i, gt_pose_i in zip(pred, gt, gt_pose):
        ...
        if use_gt_scale:
            ...
            translation_norm = torch.sqrt(translation_gt[0].dot(translation_gt[0]))
